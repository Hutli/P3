...
namespace OpenPlaylistServer.Models
{
    public class Restriction : INotifyPropertyChanged
    {
        ...
        {
            return _restrictionUnits.Where(unit => unit.Field != field) // get anything other than title restrictionunits
        ...
        private Func<Track, bool> UpdatePredicate(RestrictionType restrictionType, IEnumerable<RestrictionUnit> restrictionUnits)
        {
            ...
            Func<RestrictionUnit, Func<Track, bool>> titleFilter = unit => { return (track => track.Name.ToLower().Contains(unit.FieldValue.Trim().ToLower())); };

            var titlePred = CombineOrTrackField(titles, titleFilter);

            Func<RestrictionUnit, Func<Track, bool>> artistFilter = unit => { return (track => track.Album.Artists.Any(a => a.Name.ToLower().Contains(unit.FieldValue.Trim().ToLower()))); };
