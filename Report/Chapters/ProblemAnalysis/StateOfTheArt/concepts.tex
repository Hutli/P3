...
\label{AdministrativeEditingofPlaylist}
...
\label{AdministrativeRestrictionofPlaylist}
...
\subsection{Continuous playback}
\citetitle{sorensen2012}~\cite{sorensen2012} works with the concept of continuous music playback. This means that even though there is no track on the playlist and/or nobody has voted for anything the music should not stop playing. This concept was a main concern in the paper since the music unexpectedly stopping would otherwise deterioratete the user experience.

\subsection{Unobtrusiveness}
This concept was found in \citetitle{sorensen2012}~\cite{sorensen2012}. In the paper it was a main concept that the system should not be the centre of attention. It should supplement, rather than get in the way of, the main experience, which is social interaction. It is a system playing music in the background rather than an entertainment system. While being simple, the system should still support different levels of participation. A high level of participation should be rewarded but, to keep the system secondary, it should be possible to use the system with less effort.

\subsection{Multi-device and centralised interaction}
\label{MDCI}
The article \citetitle{sorensen2012}~\cite{sorensen2012} focuses heavily on the interaction between users and how to unify multiple devices into one system.
...
\citetitle{sorensen2012}~\cite{sorensen2012}, this is done though a
...
this project, a presentation interface is introduced to make it
