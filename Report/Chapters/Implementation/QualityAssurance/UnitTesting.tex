...
Unit tests should be quick \chnote{quick how? and in what context, making them or using them?}, meaning that a developer can verify whether or not his work is correct during development without disrupting his work flow. Unit tests should also be small, checking only a single or very small subset of the functionality of the systems code. Lastly, and perhaps most importantly, unit tests should be correct. If a unit test gives a false positive\chnote{Describe in footnote} then a developer would likely be convinced that the code which has been produced is working as it should, when in reality it is not. This could lead to a logical error breaking the application on the end users' devices. \chnote{Kun devices?} Likewise, if a unit test gives a false negative\chnote{Describe again}, a developer could waste a lot of time refactoring code, rewriting algorithms and modifying logic, which would be a very undesirable situation if the code was indeed correct, and the developer could have been using the time in a more productive manner. \chnote{fucker lang sætning men giver god mening}
...
The unit tests which were implemented covered various parts of the code, some test the correctness of methods, others verify connections to third parties \chnote{third party what?} and some verify type extensions.
...
As can be seen in \cref{fig:UnitTest}, the unit tests which were implemented in the software solution were fairly simple. The example shows the manual creation of a \textit{Track} and a call to a method, which will automatically get the \textit{Track}. The test is simply an assertion that the two \textit{Track}s are identical. In \cref{fig:UnitTestsPassed}, it can be seen that this assertion is indeed true and the test has passed. \chnote{lille snippet af fact kode?} This should mean that the method for automatically getting a \textit{Track} works as it should. \chnote{beskriv at vi får det fra en third party database, som kan opdatere sig, derfor også kræver at testen muligvis skal opdateres hvis ændret}
