From the classes it is possible to find some candidates for events, and describe how these event affect the problem domain. Also through this exercise it possible to refine classes with low cohenrence, if the classes appear to have a lot of responsiblity in the events. For events if it incorporates many different classes it is a sign for high cobling.
These are the candidates for events:
\begin{description}
